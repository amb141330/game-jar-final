 ...
 * Example: 30min game = 5 XP, 60min = 10 XP, 120min = 20 XP
 *
 * Bonus XP:
 *   First-time game:     +15 XP
 *   Co-op game:          +3 XP
 *   Weekend play:        +2 XP
 *   3 unique games/week: +25 XP
 *   7-day streak:        +50 XP
 *   Game Mastery (10th): +30 XP
 *
...
export function calculatePlayXP(game, allPlays) {
  ...
  if (prev.length === 0) {
    total += 15; bonuses.push({ label: 'First time playing!', xp: 15 });
  }
  ...
  if (day === 0 || day === 5 || day === 6) {
    total += 2; bonuses.push({ label: 'Weekend play!', xp: 2 });
  }
  ...
  if (uniqueWeek.size === 3) {
    total += 25; bonuses.push({ label: '3 unique games this week!', xp: 25 });
  }
  ...
  if (streak >= 7) {
    total += 50; bonuses.push({ label: '7-day streak! 🔥', xp: 50 });
  }

  if (prev.length === 9) {
    total += 30; bonuses.push({ label: 'Mastery: 10 plays!', xp: 30 });
  }
...
export function getBattlePassState() {
  ...
  try { const r = localStorage.getItem(BP_KEY); return r ? JSON.parse(r) : getDefault(); }
...
// NO automatic seed XP — start from 0
export function initializeFromCollection() {
  return getBattlePassState();
}
...
export function getEquippedCosmetics() {
  ...
  try { const r = localStorage.getItem(COSMETIC_KEY); return r ? { ...getDefaultCosmetics(), ...JSON.parse(r) } : getDefaultCosmetics(); }
...
// Resolve equipped cosmetic to its pool data
...
export function getCurrentTitle(level) {
  ...
  if (eq.titleBadge) {
    const reward = ALL_REWARDS.find(r => r.cosmeticId === eq.titleBadge);
